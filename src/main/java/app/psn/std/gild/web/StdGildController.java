...
public class StdGildController {
	...
	public String gildPg(Model model) {
		...
		model.addAttribute("gildMberCnt", homeService.sltGildMberList().size()); // 길드 숫자
		model.addAttribute("gildMberAccessList", homeService.sltGildMberList()); // 길드접속자 목록
		model.addAttribute("gildInfo", homeService.sltGildInfo()); // 길드정보
